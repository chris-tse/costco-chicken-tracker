...
if (!databaseUrl || databaseUrl.trim().length === 0) {
  console.error("stores:sync requires DATABASE_URL");
  process.exit(1);
}

runStoreSync(databaseUrl).catch((error) => {
  console.error("stores:sync failed");
  console.error(error);
  process.exit(1);
});

async function runStoreSync(connectionString: string): Promise<void> {
  const db = drizzle(connectionString);
  const summary = await syncStores({ db });

  console.log(formatSummary("stores:sync", summary));
}
